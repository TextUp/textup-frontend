...
export default Ember.Service.extend({
  ...
  bind(channelName, eventName, handler, options = undefined) {
  ...
  unbind() {
    ...
    if (arguments.length === 0) {
      // unbind all events on all channels
      ...
      if (arguments.length === 1) {
        ...
        // unbind specific event on a channel
  ...
  _getOrCreateSocket(options = undefined) {
  ...
  _getOrCreateChannel(socket, channelName) {
    return new Ember.RSVP.Promise((resolve, reject) => {
