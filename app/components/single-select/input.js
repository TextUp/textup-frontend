...
export default Ember.Component.extend({
	...
	actions: {
		startInput: function(event) {
			...
			if (event.type === 'keypress') { // open on enter
				if (event.which === 13) {
		...
		handleInput: function(event) {
			...
			callIfPresent(this.get('onInput'), val, event);
			if (event.which === 27 && !isEmpty) { // escape
				this.removeAndClearInput();
			} else if (event.which === 13 && !isEmpty) { // enter
				this.insertOrUpdate(event);
			} else if (event.which === 8 && (isEmpty || isBlank)) { // backspace
				this.removeAndClearInput();
			}
		},
	...
	startInput: function(event) {
		const $el = this.$(),
			...
			val = $input.val();
		$el.addClass('editing');
		callIfPresent(this.get('onInputStart'), val, event);
	...
	insertOrUpdate: function(event) {
		...
		let result;
		if (this.get('hasSelected')) {
			result = callIfPresent(this.get('onUpdate'), this.get('selected'), val, event);
		} else {
			result = callIfPresent(this.get('onInsert'), val, event);
		}
	...
	removeAndClearInput: function() {
		...
		Ember.run.next(this, function() {
			...
			if (this.get('hasSelected')) {
				callIfPresent(this.get('onRemove'), this.get('selected'));
			}
			Ember.run.scheduleOnce('afterRender', this, this.ensureFocus);
			this.setProperties({
				'publicAPI.isCreating': true,
