...
export default Ember.Component.extend({
	...
	dateOptions: computed('dateFormat', 'mMin', 'max', function() {
		...
		if (mMin) {
			...
			if (mMin.isAfter(new Date())) { // if is after now, hide 'Today'
				options.today = '';
			}
		}
		if (max) {
	...
	_isSameDay: function(moment1, moment2) {
		return isPresent(moment1) && isPresent(moment2) ?
			moment2.isSame(moment1, 'day') : false;
	},
	...
	_doUpdateTimeOptions: function() {
		...
		if (options.isValueSameDayAsMin === isValueSameDayAsMin &&
