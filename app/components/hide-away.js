...
MutObserver = window.MutationObserver || window.WebKitMutationObserver ||
...
export default Component.extend({
	...
	tabIndex: computed('disabled', 'tabindex', function() {
		return (this.get('disabled') || this.get('tabindex') === false) ?
			null :
			this.get('tabindex');
	}),
	...
	$trigger: computed(function() {
		return this.$().find('.hide-away-trigger');
	}),
	...
	removeTriggerListeners: function() {
		this.get('$trigger').off(`.${this.elementId}`);
	},
	_addBodyListeners: function($body) {
		if (this.get('_bodyHasListeners')) {
			return;
		}
		...
		if (this.get('openOn') === 'hover') {
			$body.on(`mouseleave.${this.elementId}`, function(event) {
	...
	_startObserveMutations: function($body, doReposition) {
		if (MutObserver) {
			const observer = new MutObserver(function(mutations) {
			...
			$body.on(`DOMNodeInserted.${this.elementId}`, doReposition)
	...
	_stopObserveMutations: function($body) {
		...
		if (observer) {
			...
			$body.off(`DOMNodeInserted.${this.elementId}`)
				.off(`DOMNodeRemoved.${this.elementId}`);
		}
	},
	...
	_afterOpen: function($body, skipFocus, callback) {
		...
		const bodyTimer = run.scheduleOnce('afterRender', this,
			this._addBodyListeners, $body);
	...
	_afterClose: function($body, skipFocus, callback) {
		...
		const closeTimer = run.scheduleOnce('afterRender', this, function() {
			...
			const $trigger = this.get('$trigger');
			if (!skipFocus && $trigger[0].tabIndex > -1) {
				$trigger.focus();
			}
		});
	...
	_positionVertical: function($body, {
		parentHeight,
		element: elBox,
		body: bodyBox
	}) {
	...
	_positionHorizontal: function($body, {
		parentWidth,
		element: elBox,
		body: bodyBox
	}) {
	...
	_findBody: function() {
		const $parent = this.get('floating') ? Ember.$(document) : this.$();
