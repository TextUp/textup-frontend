...
const { computed, isPresent, isNone, RSVP: { Promise }, $ } = Ember;

export default PhotoLoader.extend({
  classNames: 'stack-display',
  classNameBindings: [
    '_isLoading:loading',
    '_hasMultiple:multiple-pictures',
    '_hasTwo:two-pictures',
    'readonly'
  ],
  ...
  _doRebuild: function(toTryNext) {
    if (toTryNext === this.get('imageLinks.firstObject') || isNone(this.get('currentPreview'))) {
  ...
  _triggerRemoveForItem: function() {
    ...
    Promise.all([callIfPresent(this.get('doRemove'), this.get('_currentPreview'))]).catch(() =>
      $removeControl.removeClass('loading').addClass('error')
    );
  },
