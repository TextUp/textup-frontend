...
test('is or contains element', function(assert) {
  const $root = $(config.APP.rootElement),
...
test('wrapping an element in another element', function(assert) {
  ...
  assert.notOk($.contains($root[0], containerEl), 'container is not in DOM');

  ElementUtils.wrapElement(containerEl, childEl);

  assert.ok($.contains($root[0], containerEl), 'container is now in DOM');
  assert.ok($.contains(containerEl, childEl), 'container contains child');
});
...
test('unwrapping element', function(assert) {
  ...
  assert.ok($.contains($root[0], containerEl), 'container is in DOM');
  assert.ok($.contains($root[0], childEl), 'child is in DOM');
...
test('inserting elements within a container element', function(assert) {
  ...
  assert.ok($.contains($root[0], el1), 'el1 is in DOM');
  assert.notOk($.contains($root[0], el2), 'el2 is NOT in DOM');
  assert.notOk($.contains($root[0], el3), 'el3 is NOT in DOM');

  ElementUtils.insertElementsWithin(el1, el2, el3);

  assert.ok($.contains($root[0], el1), 'el1 is in DOM');
  assert.ok($.contains($root[0], el2), 'el2 is in DOM');
  assert.ok($.contains($root[0], el3), 'el3 is in DOM');
  assert.ok($.contains(el1, el2), 'el1 contains el2');
  assert.ok($.contains(el1, el3), 'el1 contains el3');
});
...
test('removing element from DOM', function(assert) {
  ...
  assert.notOk($.contains($root[0], el1), 'el1 is NOT in DOM');
});
