...
moduleForComponent(
  'record-cluster/item/system-note',
...
test('inputs', function(assert) {
  run(() => {
    ...
    assert.throws(() => this.render(hbs`{{record-cluster/item/system-note}}`), 'requires note');
    assert.throws(
      () => this.render(hbs`{{record-cluster/item/system-note note=item}}`),
      'requires note'
    );

    this.render(hbs`{{record-cluster/item/system-note note=note}}`);
