...
moduleForComponent(
  'record-cluster/item/call',
  'Integration | Component | record cluster/item/call',
  {
    integration: true,
    beforeEach() {
...
test('inputs', function(assert) {
  run(() => {
    ...
    assert.throws(() => this.render(hbs`{{record-cluster/item/call}}`), 'requires call');
    assert.throws(() => this.render(hbs`{{record-cluster/item/call call=rItem}}`), 'requires call');

    this.render(hbs`{{record-cluster/item/call call=rCall}}`);
