...
test('rendering a schedule', function(assert) {
  run(() => {
    ...
    this.set('scheduleObj', this.store.createFragment('schedule', initialData));
...
test('updating an existing range', function(assert) {
  run(() => {
    ...
    this.setProperties({
      scheduleClass,
      scheduleData: this.store.createFragment('schedule', initialData),
    ...
    setTimeout(() => {
      assert.equal(
        $startInput.attr('aria-expanded'),
        'true',
        'start time input open after clicking'
      );
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          assert.equal(
            $endInput.attr('aria-expanded'),
            'true',
            'end time input open after clicking'
          );
